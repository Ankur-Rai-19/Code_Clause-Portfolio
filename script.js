...
for (var i = 0; i < navMenuAnchorTags.length; i++) {
    //handel click on all of them
    navMenuAnchorTags[i].addEventListener("click", function (event) {
        // fetch an element and add event listener on them
        event.preventDefault(); // by doing this it will not take it to that section

        var targetSectionID = this.textContent
            .trim()
            .toLowerCase(); /* Getting the text of the clicked link and converting it to lowercase. */
        var targetSection =
            document.getElementById(
                targetSectionID
            ); /* Trying to get the element with the id of `targetSectionID`. */
...
by 50 pixels. */
        var interval = setInterval(function () {
            var targetSectionCoordinates =
                targetSection.getBoundingClientRect(); /* Getting the coordinates of the target section. */
...
var SkillsContainer =
    document.getElementById(
        "skills-container"
    ); /* Getting the element with the id `skills-container`. */
...
function fillBars() {
    for (let bar of progressBars) {
        ...
        let interval = setInterval(function () {
            ...
            bar.style.width =
                currentWidth +
                "%"; /* Setting the width of the progress bar to the value of the variable `currentWidth`. */
        }, 5);
    }
}
...
function checkScroll() {
    ...
    var coordinates =
        SkillsContainer.getBoundingClientRect(); /* Getting the coordinates of the element with the id `skills-container`. */
    ...
    if (!animationDone && coordinates.top <= window.innerHeight) {
        //window.innerHeight will give u the viewport height
        animationDone = true;
        fillBars();
